...
1. Copy `assets/AGENTS-rulebook.md` into the project root as `AGENTS.md` (also as `CLAUDE.md` if the tool reads that instead). Fill in its "Project Context" section. These rules are always-on and override your defaults; read them now if you haven't.
...
**Phase 2 — Build.** Smallest increments that produce runnable, testable results — diff size per acceptance is the biggest predictor of slop. Per increment: write the test first (happy path + empty/invalid input + likeliest failure), implement, tell the user exactly how to run and verify, commit. After each working step, proactively confess: any hacks, hardcoded values, skipped edge cases, or things a senior engineer would flag.
